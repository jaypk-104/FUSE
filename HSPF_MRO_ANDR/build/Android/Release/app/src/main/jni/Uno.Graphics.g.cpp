...
static void Texture2D_build(uType* type)
{
    ...
    ::STRINGS[9] = uString::Const("Texture2D");
    ::STRINGS[10] = uString::Const("Texture does not support mipmap");
    ::STRINGS[11] = uString::Const("Texture is immutable and cannot be updated");
    ::STRINGS[12] = uString::Const("GL_OES_texture_npot");
...
{
    if (IsDisposed())
        U_THROW(::g::Uno::ObjectDisposedException::New4(::STRINGS[9/*"Texture2D"*/]));
    else
...
{
    if (!IsMipmap())
        U_THROW(::g::Uno::InvalidOperationException::New5(::STRINGS[10/*"Texture doe...*/]));
    else
    {
...
{
    if (Format() == 0)
        U_THROW(::g::Uno::InvalidOperationException::New5(::STRINGS[11/*"Texture is ...*/]));
    else
    {
...
bool Texture2D::HaveNonPow2Support()
{
    ...
    {
        Texture2D::_haveNonPow2Support_ = (Texture2D::CheckGLES3Support() || Texture2D::CheckExtensionSupport(::STRINGS[12/*"GL_OES_text...*/]));
...
{
    ::STRINGS[13] = uString::Const("VideoTexture");
    type->SetInterfaces(
...
{
    if (IsDisposed())
        U_THROW(::g::Uno::ObjectDisposedException::New4(::STRINGS[13/*"VideoTexture"*/]));
    else
        ::g::OpenGL::GL::DeleteTexture(GLTextureHandle());
